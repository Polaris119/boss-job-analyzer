  ...
  if (workbenchTabId) {
    try {
      ...
      if (tab.url?.startsWith(chrome.runtime.getURL(""))) {
        const workbenchUrl = chrome.runtime.getURL(ROUTES.WORKBENCH);
        const update = tab.url === workbenchUrl ? { active: true } : { active: true, url: workbenchUrl };
        await chrome.tabs.update(workbenchTabId, update);
        return;
      }
    } catch {}
