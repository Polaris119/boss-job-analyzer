...
test("normalizeText removes duplicate whitespace", () => {
  assert.equal(parser.normalizeText("  Python\t  TypeScript\n\n\nRAG  "), "Python TypeScript\n\nRAG");
});

test("sliceByHeadings keeps only the requested job section", () => {
  const page = "导航\n职位描述\n开发 AI Agent\n熟悉 RAG\n公司介绍\n示例公司";
  assert.equal(
    parser.sliceByHeadings(page, ["职位描述"], ["公司介绍"]),
    "职位描述\n开发 AI Agent\n熟悉 RAG\n"
  );
});
...
test("splitResumeSections creates editable resume blocks", () => {
  ...
  assert.deepEqual(parser.splitResumeSections(resume), [
...
test("splitResumeSections removes adjacent duplicate page text", () => {
  const sections = parser.splitResumeSections("工作经历\n示例公司\n示例公司\n负责开发");
  assert.deepEqual(sections, [{ title: "工作经历", text: "示例公司\n负责开发" }]);
});
