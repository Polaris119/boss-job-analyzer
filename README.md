...
本项目不是 BOSS直聘官方产品，与 BOSS直聘及其关联公司不存在隶属、授权或背书关系。数据处理方式请参阅[隐私政策](./PRIVACY.md)。
...
- **定制简历**：可选生成针对当前岗位的简历，使用经典蓝色单栏模板，支持结构化编辑、区块排序、可选证件照和 PDF 导出。
...
- 定制简历的证件照由用户主动上传，仅在本机裁剪、保存和导出，不会发送给 AI 服务商。
...
- 证件照支持 JPG、PNG 和 WebP，单张不超过 5 MB；不上传照片也可以正常导出完整排版。
...
本项目采用 [MIT License](./LICENSE) 开源。随项目分发的第三方组件适用其各自的许可证，PDF.js 的许可证见 [`vendor/pdfjs/LICENSE`](./vendor/pdfjs/LICENSE)。
